...
def  ReadProfiles():
    ...
    for line in range(len(content)):
        if content[line] == '--------------------\n':
            ...
            strList.append(content[line+2])
            dexList.append(content[line+3])
            conList.append(content[line+4])
            intList.append(content[line+5])
            wisList.append(content[line+6])
            chaList.append(content[line+7])
            levelList.append(content[line+8])
            skAcroList.append(content[line+9])
            skAnHnList.append(content[line+10])
            skArcaList.append(content[line+11])
            skAthlList.append(content[line+12])
            skDeceList.append(content[line+13])
            skHistList.append(content[line+14])
            skInsiList.append(content[line+15])
            skIntiList.append(content[line+16])
            skInveList.append(content[line+17])
            skMediList.append(content[line+18])
            skNatuList.append(content[line+19])
            skPercList.append(content[line+20])
            skPerfList.append(content[line+21])
            skPersList.append(content[line+22])
            skReliList.append(content[line+23])
            skSloHList.append(content[line+24])
            skSteaList.append(content[line+25])
            skSurvList.append(content[line+26])
            stStrList.append(content[line+27])
            stDexList.append(content[line+28])
            stConList.append(content[line+29])
            stIntList.append(content[line+30])
            stWisList.append(content[line+31])
            stChaList.append(content[line+32])
        else:
            pass
...
def SaveClose(charInfo):
    ...
    for counter in charInfo:
        if counter == 0:
            pass
        else:
            str(charInfo[counter]) # THis is the issue
            charInfo[counter] += "\n"
            meg_characters.write(charInfo[counter])
            meg_characters.close()
